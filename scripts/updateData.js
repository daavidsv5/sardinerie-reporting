...
// Stores per-order product baskets (with date) instead of a pre-aggregated
// top-100 pair list, so the /crosssell page can recompute pairs for whatever
// date range is selected in the TopBar filter.

function aggregateCrossSellOrders(csv) {
  const rows = parseCSV(csv);
  ...
  // Group distinct product names + order date by order code
  const orderProducts = new Map(); // code -> { date, products: Set }
  for (const cols of rows) {
    if (cols.length < 57) continue;
    const code = cols[0];
    if (cancelledCodes.has(code)) continue;
    ...
    if (!orderProducts.has(code)) orderProducts.set(code, { date, products: new Set() });
    orderProducts.get(code).products.add(name);
  }

  return [...orderProducts.values()]
    .map(o => ({ date: o.date, products: [...o.products].sort() }))
    .sort((a, b) => a.date.localeCompare(b.date));
}

function writeCrossSellTsFile(filePath, varName, country, orders) {
  const today = new Date().toISOString().split('T')[0];
  const content = `// Auto-generated by scripts/updateData.js — last update: ${today}
// ${country.toUpperCase()}: per-order product basket (distinct product names, cancelled excluded)
...
export const ${varName}: CrossSellOrderRecord[] = ${JSON.stringify(orders, null, 2)};
`;
  fs.writeFileSync(filePath, content, 'utf8');
}
...
async function main() {
  ...
  try {
    ...
    // ── CZ cross-sell ─────────────────────────────────────────────────────────
    const crossSellCZ = aggregateCrossSellOrders(csvOrdersCZ);
    writeCrossSellTsFile(path.join(DATA_DIR, 'crossSellDataCZ.ts'), 'crossSellDataCZ', 'cz', crossSellCZ);
    const multiItemCZ = crossSellCZ.filter(o => o.products.length >= 2).length;
    log(`CZ cross-sell: ${crossSellCZ.length} orders (${multiItemCZ} multi-item)`);

    // ── SK cross-sell ─────────────────────────────────────────────────────────
    const crossSellSK = aggregateCrossSellOrders(csvOrdersSK);
    writeCrossSellTsFile(path.join(DATA_DIR, 'crossSellDataSK.ts'), 'crossSellDataSK', 'sk', crossSellSK);
    const multiItemSK = crossSellSK.filter(o => o.products.length >= 2).length;
    log(`SK cross-sell: ${crossSellSK.length} orders (${multiItemSK} multi-item)`);
